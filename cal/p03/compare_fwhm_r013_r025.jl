...
fig = Figure()
ax = Axis(fig[1, 1], xlabel = " Temperature  (K)", ylabel = "FWHM (keV)", xscale = log10, xticks = (temp, string.(temp)), #(3.4, 5.3)
                title = "LBNL-PPC01 and ASIC L1k65n with buffer:  $period, r00$(runs[1])-r0$(runs[end])", titlesize = 16)
for i in eachindex(runs)
    CairoMakie.scatter!(ax, temp[i], ustrip.(fwhm_Co60a[i]), color = colors[1], label = "$(Makie.UnicodeFun.to_superscript(60))Co a) $(peaks[1]) keV")
    CairoMakie.scatter!(ax, temp[i], ustrip.(fwhm_Co60b[i]), color = colors[2], label = "$(Makie.UnicodeFun.to_superscript(60))Co b) $(peaks[2]) keV")
end 
...
plt_name = joinpath(plt_folder, "temp_fwhm.png")
save(plt_name, fig)
