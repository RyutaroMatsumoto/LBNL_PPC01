...
phaw = [0.32, 0.16, 0.08, 0.04, 0.02, 0.01]
...
fig = Figure()
ax = Axis(fig[1, 1], xlabel = "Pulse Height Averaging Window (µs)", ylabel = "FWHM (keV)", xscale = log10, xticks = (phaw, string.(phaw)), #(3.4, 5.3)
                title = "LBNL-PPC01 and ASIC L1k65n with buffer:  $period, r00$(runs[1])-r0$(runs[end])", titlesize = 16)
for i in eachindex(runs)
    CairoMakie.scatter!(ax, phaw[i], ustrip.(fwhm_Co60a[i]), color = colors[1], label = "$(Makie.UnicodeFun.to_superscript(60))Co a) $(peaks[1]) keV")
    CairoMakie.scatter!(ax, phaw[i], ustrip.(fwhm_Co60b[i]), color = colors[end-1], label = "$(Makie.UnicodeFun.to_superscript(60))Co b) $(peaks[2]) keV")
end 
...
plt_name = joinpath(plt_folder, "phaw_fwhm.png")
save(plt_name, fig)
